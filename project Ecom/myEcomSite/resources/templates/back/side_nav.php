<div class="collapse navbar-collapse navbar-ex1-collapse">
    <ul class="nav navbar-nav side-nav">
        <li class="active">
            <a href="index.php?main"><i class="fa fa-fw fa-dashboard"></i> Dashboard</a>
        </li>


          <li class="">
            <a href="index.php?customers"><i class="fa fa-fw fa-dashboard"></i>Customers</a>
        </li>
          <li class="">
            <a href="index.php?offline_orders"><i class="fa fa-fw fa-dashboard"></i>Offline Orders</a>
        </li>

         <li class="">
            <a href="index.php?offline_reports"><i class="fa fa-fw fa-dashboard"></i>offline Reports</a>
        </li>
        <li class="">
          <a href="index.php?orders"><i class="fa fa-fw fa-dashboard"></i>Orders</a>
      </li>

       <li class="">
          <a href="index.php?reports"><i class="fa fa-fw fa-dashboard"></i>Reports</a>
      </li>
        <li>
            <a href="index.php?products"><i class="fa fa-fw fa-bar-chart-o"></i> View Products</a>
        </li>
        <li>
            <a href="index.php?add_product"><i class="fa fa-fw fa-table"></i> Add Product</a>
        </li>

        <li>
            <a href="index.php?categories"><i class="fa fa-fw fa-desktop"></i> Categories</a>
        </li>
        <li>
            <a href="index.php?brands"><i class="fa fa-fw fa-desktop"></i> Brands</a>
        </li>
        <li>
            <a href="index.php?users"><i class="fa fa-fw fa-wrench"></i>Users</a>
        </li>

    </ul>
</div>
<!-- /.navbar-collapse -->
